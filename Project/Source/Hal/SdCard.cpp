...
namespace Hal
{
	...
	SdCard::SdCard(Gpio *gpio, gpio_num_t miso, gpio_num_t mosi,
				   gpio_num_t clock, gpio_num_t cs) : 
				   _gpio(gpio), _miso(static_cast<Gpio::GpioIndex>(miso)), _mosi(static_cast<Gpio::GpioIndex>(mosi)),
				   _clock(static_cast<Gpio::GpioIndex>(clock)), _cs(static_cast<Gpio::GpioIndex>(cs))
	{
	}
